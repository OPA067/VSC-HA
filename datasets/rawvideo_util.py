...
class RawVideoExtractorCV2():
    ...
    def video_to_tensor_xpool(self, video_path, num_frames, sample, start_time, end_time, cut_video=None):
        ...
        intervals = np.linspace(start=start_time*fps, stop=end_time*fps, num=int(acc_samples) + 1).astype(int)
        ranges = []
    ...
    def get_video_data(self, video_path, start_time=None, end_time=None, sample_type=None, num_frames=12):
        if sample_type is not None:
            frames, frame_idxs = self.video_to_tensor_xpool(video_path, num_frames, sample=sample_type, start_time=start_time, end_time=end_time)
            return frames
        else:
